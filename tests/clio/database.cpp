 ...
 *  it has a singleton interface and all fetches from the database should check with the cache
 * instead of going to the intrinsic API. The cache will fetch from the database if not found.
 */
class cache
{
  public:
   ...
   class cache_record
   {
     ...
     private:
      ...
      cache_record(cache& c, db_handle h) : _cache(c), _handle(h) {}
   ...
   cache_record& create(size_t new_size)
   {
      ...
      auto& r = _db_cache
                    .emplace(std::pair<uint32_t, cache_record>(new_handle.id,
...
struct db_ref
{
   ...
   const T* operator->() const { return reinterpret_cast<const T*>(_cr.data()); }

   T* operator->() { return reinterpret_cast<T*>(_cr.modify().data()); }
...
struct db_ptr
{
   ...
   friend bool operator==(const db_ptr<T>& a, const db_ptr<T>& b) { return a.id == b.id; }
   friend bool operator!=(const db_ptr<T>& a, const db_ptr<T>& b) { return a.id != b.id; }

   const db_ref<T> operator->() const { return cache::instance().get({id}); }
   const db_ref<T> operator*() const { return cache::instance().get({id}); }
};
...
class node_cache : public std::enable_shared_from_this<node_cache<T>>
{
  public:
   ...
   void set_right(node_cptr new_right) const
   {
      ...
      if (_right)
         _right->set_parent(this->shared_from_this());
   }
   ...
   void set_left(node_cptr new_left) const
   {
      ...
      if (_left)
         _left->set_parent(this->shared_from_this());
   }
...
class tree_cache
{
  ...
  public:
   ...
   iterator lower_bound(const T& v) const { return lower_bound(v, root()); }
   iterator end() const { return iterator(node_cptr()); }

  private:
   int height(const node_cptr& t) const { return t ? t->height() : -1; }
    ...
    */
   void set_root(const node_cptr& r) const { const_cast<tree_cache*>(this)->set_root(r); }
   void set_root(const node_cptr& r)
   {
      ...
      {
         std::cout << "_root->ptr.id(): " << _root->ptr().id << "  self->root.id: " << self->root.id
                   << "  value: " << _root->value() << "\n";
