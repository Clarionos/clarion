...
TEST_CASE( "flatptr" ) {
   ...
   constexpr uint32_t offsetb = clio::get_tuple_offset<1,std::tuple<sub_obj,clio::flat_ptr<sub_obj>> >::value;
   ...
   std::cout << "contains_offset_ptr: " << clio::contains_offset_ptr<sub_obj>() <<"\n";
    ...
    /*
    clio::flat_ptr<root> r( root{ .other = "other", .fatptr = fat{ .a = 0x0A, .b = 0x0B, .substr = "sub" } } );
    std::cout << "r.size: " << r.size() << "\n";
    std::vector<char> d( r.data(), r.data()+r.size() );
