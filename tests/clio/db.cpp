...
class cache
{
  public:
   class cache_record
   {
     public:
      ...
      cache_record(cache_record&& mv)
          : _cache(mv._cache), _handle(mv._handle), _buffer(std::move(mv._buffer)) _dirty(mv._dirty)
      {
         _mv.handle.id = -1;
      }
   ...
   cache_record& db_create(L&& constructor)
   {
      ...
      auto& r = _db_cache[new_handle.id] =
          cache_record(*this, new_handle).modify(std::forward<L>(constructor));
      _dirty.push_back(&r);
   }
...
{
  public:
   const char* data() const { return _get_record().data(); }
   size_t size() const { return _get_record().size(); }
