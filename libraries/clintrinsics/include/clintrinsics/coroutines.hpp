...
namespace clintrinsics
{
   ...
   struct TaskBase
   {
      ...
      TaskBase(TaskBase&& src) { *this = std::move(src); }
      ...
      // but that causes clang 11.0.0 to report an error: "WebAssembly 'tail-call' feature not
      // enabled". This will also require changes to promise_type::final_suspend().
