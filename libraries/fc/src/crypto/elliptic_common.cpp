...
namespace fc { namespace ecc {
    ...
    {
      uint32_t check = (uint32_t)sha256::hash(key.data, sizeof(key))[0];
    ...
    public_key public_key::from_base58( const std::string& b58 )
    {
        ...
        public_key_data key;
        uint32_t check = (uint32_t)sha256::hash(data.data, sizeof(key))[0];
    ...
    unsigned int public_key::fingerprint() const
    {
        ...
        unsigned char* fp = (unsigned char*) hash.data();
